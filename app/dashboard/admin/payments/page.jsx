...
export default function AdminPaymentsPage() {
  ...
  const list = useApi(() => admin.payments.list({ status, search }), {
    as: "admin",
    deps: [status, search],
  });
          ...
          <div className="mt-6 overflow-x-auto">
            <table className="w-full min-w-220 text-left text-sm text-slate-700">
              <thead className="border-b border-slate-200 text-xs uppercase tracking-[.16em] text-slate-400"><tr>
                <th className="px-4 py-3">Student</th><th className="px-4 py-3">Payment ID</th><th className="px-4 py-3">Amount</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Receipt</th>
              </tr></thead>
              <tbody>
                {payments.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-slate-500">No payments match this filter.</td></tr>
                )}
                {payments.map((payment) => <tr key={payment.id} className="border-b border-slate-100 hover:bg-slate-50">
